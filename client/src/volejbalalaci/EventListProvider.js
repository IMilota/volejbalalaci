...
function EventListProvider({ children }) {
  ...
  async function handleLoad() {
    ...
    const response = await fetch(`api/volejbalalaci/event/list`, {
      method: "GET",
    });
  ...
  async function handleCreate(dtoIn) {
    setEventLoadObject((current) => ({ ...current, state: "pending" }));
    const response = await fetch(`api/volejbalalaci/event/create`, {
      method: "POST",
      headers: {
  ...
  async function handleUpdate(dtoIn) {
    setEventLoadObject((current) => ({ ...current, state: "pending" }));
    const response = await fetch(`api/volejbalalaci/event/update`, {
      method: "POST",
  ...
  async function handleAttendance(dtoIn) {
    setEventLoadObject((current) => ({ ...current, state: "pending" }));
    const response = await fetch(`api/volejbalalaci/attendance/update`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(dtoIn),
    });
